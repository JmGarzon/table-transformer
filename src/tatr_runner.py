...
DETECTION_MODEL_PATH = r"C:\Users\jmgarzonv\Desktop\EAFIT\Tesis\table-transformer\pretrained_models\pubtables1m_detection_detr_r18.pth"

STRUCTURE_CONFIG_PATH = "src\structure_config.json"
STRUCTURE_MODEL_PATH = "pretrained_models\TATR-v1.1-All-msft.pth"
...
def cells_to_html(cells):
    ...
    for cell in cells:
        ...
        if this_row > current_row:
            ...
            row = ET.SubElement(tbody if not cell['column header'] else thead, "tr")
...
class TableTransformerRunner:
    def __init__(self, mode = "structure" # "detection", "structure" or "extraction" (both)
):
        assert mode in ["detection", "structure", "extract"], "Invalid mode, must be 'detection', 'structure' or 'extract'."
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.tokens = [] # No OCR tokens available
        self.mode = mode
        # Create inference pipeline
        print(f"Creating inference pipeline in mode '{mode}'")
        if mode == "detection":
            self.pipe = TableExtractionPipeline(det_device=device,
                                    str_device=device,
    ...
    def predict(self, img):
        ...
        if self.mode == 'structure':
            extracted_table = pipe.recognize(img, tokens, out_objects=out_objects, out_cells=out_csv,
                                out_html=out_html, out_csv=out_csv)
            # print("Table(s) recognized.")
            return extracted_table["html"][0]


        elif self.mode == 'detection':
            out_objects = out_crops = True
            detected_tables = pipe.detect(img, tokens, out_objects=out_objects, out_crops=out_crops)
            print("Table(s) detected.")
            print(detected_tables)


        elif self.mode == 'extract':
            extracted_tables = pipe.extract(img, tokens, out_objects=out_objects, out_cells=out_csv,
                                            out_html=out_html, out_csv=out_csv,
                                            crop_padding=crop_padding)
            # print("Table(s) extracted.", extracted_tables)
            try:
                return extracted_tables[0]["html"][0]
            except:
